    ...
    return pl.DataFrame(
        {
            ...
            "signup_date": [
                date(2023, 1, 1) + timedelta(days=rng.randint(0, 900)) for _ in range(n)
            ],
        }
    )
...
def _orders(
    n: int, customer_ids: list[int], products: pl.DataFrame, rng: random.Random
) -> pl.DataFrame:
    prod_ids = products["product_id"].to_list()
    prices = dict(zip(prod_ids, products["unit_price"].to_list(), strict=True))
    rows = []
...
def main() -> None:
    ...
    rng = random.Random(args.seed)  # noqa: S311 — demo data, cryptography değil
